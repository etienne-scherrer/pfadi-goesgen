...
class EventsHandler extends AbstractExtJSHandler {
  ...
  function __construct() {
    ...
    $this->eventDbTableColumns=array(
        'event_nr' => 'event_nr',
        'text_nr' => 'text_nr',
  ...
  public function handleUpdateRequest($request, $n = 0) {
    ...
    $n = $n + $this->db->update('am_text', array_intersect_key($request->data, $this->textDbTableColumns), array('text_nr = ?' => $id));
    //update events part
  ...
  public function handleDeleteRequest($request) {
    ...
    $n = $this->db->delete('am_text', array('text_nr = (SELECT text_nr FROM am_event WHERE event_nr = ?)' => $id));
    if($n == 0){
  ...
  protected function createReadFrom($select) {
    $select->from(array('e' => 'am_event'))->join(array('t' => 'am_text'), 'e.text_nr = t.text_nr');
  }
}
